...
echo "🚀 INIT-CERBOS.SH started..."

SCHEMA_EXISTS=$(psql -h macarons -U "$POSTGRES_USER" -d "$POSTGRES_DB" -tAc \
"SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'cerbos'")

if [[ "$SCHEMA_EXISTS" == "cerbos" ]]; then
  echo "Schema 'cerbos' already exists. Skipping initialization."
else
  echo "Creating schema and initializing Cerbos..."

  psql -h macarons -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 <<'EOSQL'
...
EOSQL
  ...
  echo "✅ Cerbos schema initialized."
fi
